...
export interface SandboxOpts {
  ...
  cwd?: string             // 不传则用 hook 所属项目目录（与 Claude Code 一致；见 2026-08-02 修正）
...
- **环境隔离**：子进程 `env` 只传白名单（PATH、HOME、必要的 CLAUDE_* 模拟变量如 effort），**不透传**本进程全部 env（防泄漏 token）。cwd = hook 所属项目目录（**2026-08-02 修正**：原设计用临时空目录，导致 `.claude/hooks/x.sh` 这类相对命令一律 exit 127——本仓库 4 个 hook 有 3 个中招；真实 Claude Code 就在项目目录跑 hook，不还原它 dry-run 毫无意义）。同时把 `CLAUDE_PROJECT_DIR` 放进白名单，脚本用它拼路径才成立。
...
> **明确不做**：不用 Docker/VM 级沙箱（过重）。隔离手段 = spawn + env 白名单 + 超时硬杀 + 输出上限 + 网络默认关（cwd 不再隔离——见上）。UI 必须警示"dry-run 仍会在你机器上真实执行该命令"。
...
- **安全确认**：command 类点"运行"前弹确认，明示"将在你的项目目录真实执行该命令"；http 类需勾"允许出网"。
...
- [x] 2. `electron/services/hook-sandbox.ts`：`dryRunHook` + 三形态执行 + 安全边界（spawn/stdin/超时/env 白名单/项目 cwd/输出上限/决策解析）。
...
- [x] dry-run 的 cwd 是 hook 所属项目目录（hook 内 `pwd` 验证；2026-08-02 修正后语义反转，理由见改动方案）。
- [x] Web 模式调用 dryRun 抛"仅桌面端"，不崩。

## 风险与备注

- **dry-run 仍是真实执行**：沙箱只隔离 env/超时/网络（cwd 就是真实项目目录），**不能阻止 hook 命令本身的副作用**（它若写 `~/.bashrc` 照样写）。这是命令型 hook 的本质，UI 必须显著警示，不可宣称"绝对安全"。Docker 级隔离过重不做。
